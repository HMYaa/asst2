 ...
 * - next_task / remaining：与 Part A 线程池里「抢 task 下标 + 还剩几个」同构，只是按 launch 隔离。
 * - unfinished_deps：注册时设为「尚未完成的、且仍 blocking 的前驱边」条数；某前驱整批完成时
 *   对每条出边做一次 fetch_sub，减到 0 表示该后继可进 ready_queue_。
 */
struct BulkLaunch {
    ...
    BulkLaunch(IRunnable* r, int n) : runnable(r), num_tasks(n), next_task(0), remaining(n), unfinished_deps(0) {}
};
 ...
 *
 * 4) ready 队列里同一 lid 可出现多次：抢不到合法 start 但 remaining>0 时会 push_back 回去，属性能折中。
 */
...
// 仅启动 worker；图与队列在第一次 runAsyncWithDeps 时增长
...
// 假定已持有 graph_mtx_：遍历 successors_[launch_id]，对每个后继 unfinished_deps--，到 0 则入 ready
void TaskSystemParallelThreadPoolSleeping::complete_bulk_launch_unlocked(TaskID launch_id) {
    ...
    for (TaskID succ : successors_[launch_id]) {
        ...
        // 返回减一前的值；为 1 表示减后变 0，后继所有前驱都已满足
...
void TaskSystemParallelThreadPoolSleeping::worker_loop() {
    while (true) {
        ...
        // 零任务 launch：不跑 runTask，但仍要触发「本节点完成」以解开依赖图
        if (n == 0) {
            complete_bulk_launch(lid);
            continue;
        }

        // 与 Part A Sleeping 相同动机：少抢全局原子，降 next_task 缓存行争用
        int chunk = (n + num_workers_ - 1) / num_workers_;
        if (chunk < 1) {
            chunk = 1;
        }
        if (chunk > 32) {
        ...
        while (true) {
            const int start = L.next_task.fetch_add(chunk, std::memory_order_relaxed);
            if (start >= n) {
                // 号被抢光但同批仍有 task 在执行：把 lid 扔回队列，避免空闲线程干等
                if (L.remaining.load(std::memory_order_acquire) > 0) {
                    std::lock_guard<std::mutex> lk(graph_mtx_);
            ...
            for (int task_id = start; task_id < lim; ++task_id) {
                L.runnable->runTask(task_id, n);
                // fetch_sub 返回减之前的值；从 1 减到 0 表示本 bulk 最后一个 task
...
TaskID TaskSystemParallelThreadPoolSleeping::runAsyncWithDeps(IRunnable* runnable, int num_total_tasks,
...

    {
        ...
        launches_.emplace_back(new BulkLaunch(runnable, num_total_tasks));
        id = static_cast<TaskID>(launches_.size() - 1);
        successors_.resize(launches_.size());

        // 只把「注册时仍未结束的」前驱记入边；已完成的 dep 不占 unfinished_deps
        int unresolved = 0;
        for (TaskID d : deps) {
            assert(d < id);
    ...
    // 必须在释放 graph_mtx_ 之后再 notify，否则 worker 可能被挡在锁外
    if (schedule_now) {
        cv_work_.notify_all();
    }
